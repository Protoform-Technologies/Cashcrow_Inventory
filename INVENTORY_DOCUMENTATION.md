...
### Key Components:
- **Product Management Actions (`src/actions/products.ts`)**: Handles the complex lifecycle of a product, including file processing, notifications, and path revalidation.
- **Inventory Service Layer (`src/lib/inventory.ts`)**: Provides standardized, cached read/write operations for the `products` table.
- **Shared UI Components (`src/components/shared/inventory/`)**: A modular library of components used for both grid-based browsing and deep-dive product details.
...
- **Product Photos**: Stored in the `products` bucket.
- **Technical Data Sheets**: PDF documentation stored in the `data_sheets` bucket, allowing users to view specifications directly from the product page.
...
**The Mutation Flow:**
1. **Action Receipt**: Receives `FormData` containing text fields and binary files (Photo/PDF).
2. **Storage Processing**: Files are uploaded to Supabase Storage, and public URLs are generated.
3. **Database Write**: The clean metadata + public URLs are sent to `src/lib/inventory.ts` for insertion/update.
4. **Cache Invalidation**: `revalidatePath()` is called to ensure all dashboard stats and inventory lists are instantly updated.
5. **Notification Trigger**: System alerts are fired for events like "New Product Added" or "Item Out of Stock".
...
| **Product Card** | Grid visualization | High-density information display including stock progress bars. |
...
Product availability is tightly coupled with **Supplier Lead Times**:
...
- **Vendor Terms**: Financial liquidity is tracked through payment terms like **Immediate** or Net 30, which are documented in the Supplier Management module.
- **Logistics Badges**: Inventory managers can quickly identify the reliability of a part's source by the colored delivery badges synced across the platform through the **Supplier Logistics Logic**.
