...
export async function sendOnboardingEmail(email: string, firstName: string, lastName: string) {
    const fromEmail = process.env.DOMAIN
    const fullName = `${firstName} ${lastName}`

    try {
        const { data, error } = await resend.emails.send({
            from: `Cashcrow Inventory <onboarding@${fromEmail}>`,
            to: [email],
            ...
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; rounded: 12px;">
                    <div style="text-align: center; margin-bottom: 24px;">
                        ...
                        <p style="color: #64748b; font-size: 14px; margin-top: 4px;">Inventory Management</p>
                    </div>
                    
                    <h2 style="color: #0f172a; margin-top: 0;">Welcome to the Team, ${firstName}!</h2>
                    <p style="color: #334155; line-height: 1.6;">Your account has been successfully created. You can now log in to manage parts, suppliers, and inventory movements.</p>
...
/**
 * Send an account reactivation email
 */
export async function sendReactivationEmail(email: string, firstName: string, lastName: string) {
    const fromEmail = process.env.DOMAIN

    try {
        const { data, error } = await resend.emails.send({
            from: `Cashcrow Inventory <notifications@${fromEmail}>`,
            to: [email],
            subject: 'Account Reactivated | Cashcrow Inventory',
            html: `
                <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; rounded: 12px;">
                    <div style="text-align: center; margin-bottom: 24px;">
                        <h1 style="color: #265136; margin: 0;">Cashcrow</h1>
                    ...
                    <h2 style="color: #0f172a; margin-top: 0;">Welcome Back, ${firstName}!</h2>
                    <p style="color: #334155; line-height: 1.6;">Good news! Your account has been reactivated by the administrator. You now have full access to the laboratory inventory system again.</p>
                    
                    <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 16px; border-radius: 8px; margin: 24px 0;">
                        <p style="margin: 0; font-size: 14px; color: #166534; font-weight: 600;">Status: Account Active</p>
                        <p style="margin: 4px 0 0 0; font-size: 14px; color: #15803d;">You can log in using your existing credentials.</p>
                    </div>
                    ...
                    <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 32px 0;" />
                    <p style="color: #94a3b8; font-size: 12px; text-align: center; margin: 0;">
                        If you did not expect this or have any issues, please contact your administrator.
                    </p>
                </div>
            `
        })
